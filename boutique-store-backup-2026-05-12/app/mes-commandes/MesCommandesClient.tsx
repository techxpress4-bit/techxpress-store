    ...
    return (
      <div className="pt-28 pb-20 min-h-screen flex items-center justify-center">
        <div className="text-center max-w-sm mx-auto px-4">
          ...
          >
            <svg className="w-9 h-9 text-[#6b3fa0]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <div
              key={order.id}
              className="card p-5 hover:border-[rgba(107,63,160,0.4)] transition-colors"
            >
              {/* Header */}
              ...
              {/* Status info for en_attente */}
              {order.statut === "en_attente" && (
                <div
                  className="mt-4 p-3 rounded-xl text-xs text-[#9ca3af] flex items-center gap-2"
                  style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.2)" }}
                >
                  <svg className="w-3.5 h-3.5 flex-shrink-0 text-[#f59e0b]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
