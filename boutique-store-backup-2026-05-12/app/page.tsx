      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-24">
          ...
          <h1
            ...
            style={{ fontFamily: "var(--font-syne)", lineHeight: 1.1, animationDelay: "0.1s" }}
          >
            Votre destination{" "}
          ...
          <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              >
                {/* Glow corner */}
                <div
                  className="absolute top-0 right-0 w-20 h-20 rounded-full opacity-20 blur-2xl pointer-events-none transition-opacity duration-300 group-hover:opacity-40"
                ...
                <p className="text-sm font-bold text-white leading-snug" style={{ fontFamily: "var(--font-syne)" }}>
                  {f.title}
                </p>
      ...
      {featured && featured.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/catalogue" className="btn-ghost text-sm hidden sm:flex">
              Voir tout →
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-20">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="section-title">Parcourez nos rayons</h2>
          </div>
          <Link href="/catalogue" className="btn-ghost text-sm hidden sm:flex">
            Tout voir →
          </Link>
          ...
          {categories.length > 0
            ? categories.map((cat) => (
                <Link
                  ...
                  className="group card p-4 flex flex-col items-center text-center gap-3 hover:border-violet"
                >
                  <span className="text-3xl group-hover:scale-110 transition-transform duration-200">
                    {cat.icone || categoryIcons[cat.slug.current] || "📦"}
                  </span>
                  <span className="text-xs font-semibold text-[#9ca3af] group-hover:text-white transition-colors leading-tight" style={{ fontFamily: "var(--font-syne)" }}>
                    {cat.nom}
                  </span>
                </Link>
              ))
            : ["Box TV Android", "Abonnements TV", "Accessoires Tél.", "Routeurs", "Câbles", "Supports TV", "Paraboles"].map(
                (name, i) => (
                  <div key={i} className="card p-4 flex flex-col items-center text-center gap-3 animate-pulse">
