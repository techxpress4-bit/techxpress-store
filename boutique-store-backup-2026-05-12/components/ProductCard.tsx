...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="group card flex flex-col overflow-hidden cursor-pointer"
      style={{
        background:
    ...
    >
      ...
      <div className="relative aspect-square bg-[#111] overflow-hidden">
        ...
        {/* Overlay on hover */}
        <div className="absolute inset-0 bg-[rgba(107,63,160,0.15)] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

        {/* Badges */}
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          <span
            className={`badge-stock ${product.enStock ? "available" : "unavailable"}`}
          >
        ...
        {product.optionAbonnement && (
          <div className="absolute top-3 right-3">
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold" style={{ background: "rgba(107,63,160,0.3)", color: "#c084fc", border: "1px solid rgba(192,132,252,0.3)" }}>
              + Abonnement
            </span>
          </div>
        )}
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3 className="text-sm font-semibold text-[#f5f5f5] leading-snug line-clamp-2 group-hover:text-white transition-colors" style={{ fontFamily: "var(--font-syne)" }}>
            {product.nom}
          </h3>
        </div>

        {product.categorie && (
          <span className="text-xs text-[#6b7280] mb-3">
