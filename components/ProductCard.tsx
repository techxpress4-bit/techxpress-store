...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Image — fond beige sur mobile (mix-blend fonctionne), fond clair sur desktop (photos fond blanc seamless) */}
      <div className="relative aspect-square bg-[#f7f3ee] sm:bg-[#fafafa] overflow-hidden">
        {imageUrl ? (
          <Image
            src={imageUrl}
            alt={product.nom}
            fill
            className="object-cover card-product-img transition-transform duration-500 group-hover:scale-105"
            sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
          />
        ) : (
          <div className="absolute inset-0 flex items-center justify-center">
            <svg className="w-16 h-16 text-[#ccc]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3
            className="text-sm font-semibold text-[#1c1917] sm:text-[#f5f5f5] leading-snug line-clamp-2 sm:group-hover:text-white transition-colors"
        ...
        {product.categorie && (
          <span className="text-xs text-[#a8998a] sm:text-[#6b7280] mb-3">
